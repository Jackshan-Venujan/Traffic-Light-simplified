...
```
Traffic------new/
├── traffic-light-model.ipynb     # Main notebook — all 23 cells
├── yolov8s.pt                    # YOLOv8s pretrained weights (COCO)
├── yolo11n.pt                    # YOLOv11n weights (for reference / Cell 23 next steps)
├── Dataset/                      # Raw LISA dataset (Supervisely JSON format)
│   ├── meta.json
│   ├── train/
│   │   ├── ann/                  # 20,535 annotation files (.jpg.json)
│   │   ├── img/                  # 20,535 images (1280×960)
│   │   └── meta/
│   └── test/
│       ├── ann/                  # 22,481 annotation files (.jpg.json)
│       ├── img/                  # 22,481 images
│       └── meta/
├── yolo_dataset/                 # Auto-generated by notebook — YOLO-format data
│   ├── images/
│   │   ├── train/
│   │   ├── val/
│   │   └── test/
...
├── class_distribution.png        # Cell 6 output
├── bbox_analysis.png             # Cell 7 output
├── class_samples.png             # Cell 5 output
├── sample_images_with_boxes.png  # Cell 8 output
├── yolo_label_check.png          # Cell 15 output
└── output.png                    # Cell 17 training curves
```
...
```
Dataset/
├── meta.json          ← Defines all 14 class names
├── train/
│   ├── ann/           ← 20,535 annotation files (one .jpg.json per image)
│   ├── img/           ← 20,535 JPEG images at 1280×960 pixels
│   └── meta/          ← Empty — ignored
└── test/
    ├── ann/           ← 22,481 annotation files
    ├── img/           ← 22,481 JPEG images
    └── meta/
```
...
### 3.3 Class Mapping — 14 Raw Classes to 3 Semantic Classes
...
## 4. Pipeline — All 23 Notebook Cells

This section explains every cell in `traffic-light-model.ipynb`: what it does and **why** it is necessary. Each cell builds on the previous one; skipping cells will cause errors.

---

### Cell 1 — Install Libraries

**What:** Installs four Python packages: `ultralytics`, `tqdm`, `PyYAML`, `onnx`.
...
**What:** Imports all libraries used throughout the notebook. Defines every file path as a `pathlib.Path` variable. Counts and prints the number of images and annotations in train and test to verify the dataset loaded correctly (expected: 20,535 train, 22,481 test).

**Why:** Centralising all paths in one cell prevents hardcoded string errors later. If the dataset is moved or renamed, only this cell needs updating. `pathlib.Path` handles cross-platform path separators automatically.

---

### Cell 3 — Understand Annotation Format

**What:** Loads a single `.jpg.json` annotation file and prints its raw content. Then explains each field line by line.

**Why:** The LISA dataset was downloaded in Supervisely JSON format, which is different from the original LISA CSV format most tutorials describe. Before writing any parsing code, it is essential to see exactly what the raw data looks like. This cell is purely educational — it produces no output files.
